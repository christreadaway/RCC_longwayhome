...
export default function TitleScreen() {
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center bg-gradient-to-b from-trail-cream to-trail-parchment px-4">
      ...
      <div className="relative z-10 text-center max-w-xl w-full">
        <h1 className="text-5xl md:text-7xl font-bold text-trail-darkBrown mb-3 font-display">
          The Long Way Home
        </h1>
        <p className="text-trail-brown text-xl mb-10 italic">
          A journey of faith, choice, and consequence
        </p>

        {mode === 'title' && (
          <div className="space-y-4">
            <button onClick={() => setMode('join')} className="btn-primary w-72 text-xl py-4">
              Join a Session
            </button>
            <br />
            <button onClick={handlePlayOffline} className="btn-secondary w-72 text-xl py-4">
              Play Offline
            </button>
            <br />
            <button
              ...
              className="text-trail-blue underline hover:text-trail-darkBlue text-base mt-4 inline-block"
            >
              Resume Saved Game
            </button>
          </div>
        ...
        {mode === 'join' && (
          <form onSubmit={handleJoin} className="card max-w-md mx-auto space-y-4">
            <h2 className="text-2xl font-semibold text-trail-darkBrown">Join Session</h2>
            <div>
              <label className="block text-base text-trail-brown mb-1">Session Code</label>
              <input
                type="text"
                value={sessionCode}
            ...
            <div>
              <label className="block text-base text-trail-brown mb-1">Your Name</label>
              <input
                type="text"
                value={studentName}
            ...
            {error && <p className="text-trail-red text-base">{error}</p>}
            <div className="flex gap-3 justify-center">
              <button type="button" onClick={() => setMode('title')} className="btn-secondary text-lg px-6 py-2">
                Back
              </button>
              <button type="submit" disabled={loading} className="btn-primary text-lg px-6 py-2">
        ...
        {mode === 'title' && error && (
          <p className="text-trail-red text-base mt-4">{error}</p>
        )}

        <p className="text-trail-brown/50 text-sm mt-12">
